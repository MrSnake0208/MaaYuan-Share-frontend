...
export interface AnnouncementSectionMeta extends Omit<
  Required<AnnouncementSectionMetaRaw>,
  "time"
> {
  time: Date;
}
...
export function parseAnnouncement(raw: string): Announcement {
  ...
  const sections: (AnnouncementSection | undefined)[] = rawSections.map((rawSection) => {
    try {
      ...
      const title =
        segments[0]?.replace(/^#+/, "").trim() || i18n.models.announcement.default_title;
