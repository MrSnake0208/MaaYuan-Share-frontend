    ...
    return {
      ...
      opers: operation.opers?.map((operator) => {
        ...
        if (typeof moduleValue === "number") {
          ...
          actualModule =
            key in CopilotDocV1.Module ? CopilotDocV1.Module[key] : CopilotDocV1.Module.Default;
        }
...
function normalizeOperation(operation: CopilotDocV1.Operation): CopilotDocV1.Operation {
  ...
  if (actions && !Array.isArray(actions) && typeof actions === "object") {
    const simingActions = operation.simingActions ?? (actions as CopilotDocV1.SimingActionMap);
    return {
      ...operation,
      actions: undefined,
      simingActions,
