...
import { RoundActionsInput, editorActionsToRoundActions } from "./action/roundMapping";
import {
...
const DETECTION_ROI: Readonly<[number, number, number, number]> = [641, 50, 43, 27];
...
function cloneConfig(config: SimingActionConfig | undefined): SimingActionConfig | undefined {
  if (!config) {
    return undefined;
  }
...
function createCustomDelayNode(delays: SimingActionDelays): SimingActionConfig {
  const normalize = (value: number | undefined, fallback: number) => {
    ...
    const normalized =
      Number.isFinite(numberLike) && numberLike >= 0 ? Math.round(numberLike) : fallback;
...
function buildRoundNodes(
  ...
  if (entries.length === 0) {
    result[RESTART_NODE_KEY] = ensureNextArray(cloneConfig(RESTART_NODE) ?? { next: [] });
    return result;
  }
  ...
  entries.forEach(([roundKey, actions], index) => {
    ...
    if (roundsWithOrangeRestart.has(roundKey)) {
      ...
      const orangeConfig = ensureNextArray(cloneConfig(ORANGE_DETECTION_TEMPLATE) ?? { next: [] });
    ...
    actionList.forEach((entry) => {
      ...
      if (token.startsWith("额外:")) {
        ...
        const baseConfig = cloneConfig(templateKey ? ACTION_TEMPLATES[templateKey] : undefined) ?? {
          action: "Click",
          post_delay: resolveDelayForTemplate(templateKey, delays) ?? DEFAULT_SIMING_ATTACK_DELAY,
        };
...
export function simingActionsToRoundActions(
  ...
  const actionEntries = Object.entries(simingActions)
    ...
    .sort((a, b) =>
      a.roundKey === b.roundKey ? a.order - b.order : Number(a.roundKey) - Number(b.roundKey),
    );
...
function sanitizeSimingActionDelays(delays: SimingActionDelays | undefined): SimingActionDelays {
  const normalize = (value: number | undefined, fallback: number) => {
    const numberLike = Number(value);
...
export async function toSimingOperationRemote(
  ...
  const baseLevelRecognition =
    (baseOperation as any).level_recognition_name ?? (baseOperation as any).levelRecognitionName;
  const normalizedLevelRecognition =
    [editorLevelRecognition, baseLevelRecognition]
  ...
  const ensureActivityRecognitionName = () => {
    const trimmed =
      typeof payload.level_recognition_name === "string"
        ...
        : "";
    payload.level_recognition_name = trimmed.length > 0 ? trimmed : fallbackActivityRecognitionName;
  };
  const payload: any = {
  ...
  } else if (opts?.level?.catOne === "家具" || opts?.level?.catOne === "其他") {
