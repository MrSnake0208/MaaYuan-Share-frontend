...
export const EditorPreviewTrigger: FC<{ buttonProps?: any }> = ({
  buttonProps,
}) => {
  const t = useTranslation()
  const { isMD, isLG } = useCurrentSize()
  const [open, setOpen] = useState(false)
  const operation = usePreviewOperation()
  const { data: levels } = useLevels()

  const previewLabel = t.components.editor2?.EditorToolbar?.preview || '预览'
  ...
  return (
    <>
      <Drawer
        ...
        className={isMD ? undefined : 'max-w-[1100px]'}
      >
        <DrawerLayout
          title={
            <>
      ...
      <Button
        icon="eye-open"
        text={isLG ? undefined : previewLabel}
        title={isLG ? previewLabel : undefined}
        aria-label={isLG ? previewLabel : undefined}
        {...buttonProps}
