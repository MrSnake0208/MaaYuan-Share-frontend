...
function setDiscSlot(
  ...
  const nextSlots = prev.map((s) => (s.index === slotIndex ? { ...s, ...updates } : { ...s }));
  ...
  const { discsSelected, discStarStones, discAssistStars } = syncLegacyArraysFromSlots(nextSlots);
...
function getStats(
  ...
  return {
    ...
    starLevel:
      s?.starLevel ??
      (typeof (operator as any).starLevel === "number" ? (operator as any).starLevel : undefined) ??
      0,
    attack:
      s?.attack ?? (typeof (operator as any).attack === "number" ? (operator as any).attack : 0),
...
export const OperatorItem: FC<OperatorItemProps> = memo(
  ({ operator, onRemove, onChange, onOverlay, isDragging, attributes, listeners }) => {
                ...
                : controlsEnabled &&
                  Array.from({ length: skillCount }, (_, index) => {
                    ...
                    const skillLevel = selected
                      ? (requirements.skillLevel ?? getDefaultRequirements(info?.rarity).skillLevel)
                  ...
                  <Select
                    ...
                    items={[
                      ...
                      ...info.modules
                        .map((m) =>
                          m
                            ? (CopilotDocV1.Module[m] as CopilotDocV1.Module | undefined)
