...
import { DEFAULT_SIMING_ACTION_DELAYS, SimingActionDelays } from "./siming/constants";
import {
  EditorAction,
  EditorOperation,
  EditorOperator,
...
export function toEditorOperation(source: CopilotOperationLoose): EditorOperation {
  ...
  const simingActions = (operation as { simingActions?: CopilotDocV1.SimingActionMap })
    .simingActions;
  const originalActions = Array.isArray(operation.actions) ? operation.actions : [];
  ...
  if (convertedActions.length === 0 && simingActions && Object.keys(simingActions).length > 0) {
