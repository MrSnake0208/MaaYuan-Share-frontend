import { Button, ButtonGroup, Card, NonIdealState, Tag } from "@blueprintjs/core";
...
import { RoundActionsInput, editorActionsToRoundActions } from "../editor2/action/roundMapping";
...
export const ActionSequenceViewer: FC<ActionSequenceViewerProps> = ({
    ...
    <Card key={round} className="card-shadow-subtle space-y-3 !p-4">
      ...
      <div className="flex flex-wrap gap-2">
        {tokens.map(({ raw, label, key }) => (
          <Tag key={key} large intent="primary" minimal title={raw !== label ? raw : undefined}>
            {label}
          </Tag>
        ))}
      </div>
  ...
  const renderTableView = () => {
    ...
    const assignedSlots = SLOT_KEYS.filter((slot) => Boolean(slotAssignments[Number(slot)]?.name));
    ...
    const slotsToRender =
      assignedSlots.length > 0
        ? SLOT_KEYS.filter((slot) => assignedSlots.includes(slot) || slotsWithTokens.has(slot))
    ...
    const noActionsText = t.components.viewer.OperationViewer.action_table_no_actions;
    const otherActionsText = t.components.viewer.OperationViewer.action_table_other_actions;
          ...
          <div className="inline-block min-w-full align-middle">
            <div className="rounded-md border border-gray-200 dark:border-gray-600 overflow-hidden">
              <table className="min-w-full border-collapse">
                      ...
                      return (
                        ...
                        >
                          <div className="flex flex-col items-center gap-2">
                            ...
                            <div className="text-sm font-semibold text-center">{slotLabel}</div>
                            <div className="text-xs text-gray-500 dark:text-gray-400">
                              {slotPosition}
                            </div>
                          </div>
                ...
                <tbody>
                  {tableRows.map((row) => (
                    <tr key={row.round} className="bg-white dark:bg-slate-800/60">
                            ...
                            {slotTokens.length > 0 ? (
                              <div className="flex flex-wrap gap-2 justify-center">
                                {slotTokens.map((token) => {
                                  ...
                                  const summary = formatTokenSummary(token.raw, language);
                                  return (
                                    <div
                                      key={token.key}
...
function symbolToActionLabel(symbol: BasicActionSymbol, language: Language): string {
  if (language === "zh_tw") {
...
export function formatTokenSummary(rawToken: string, language: Language): string {
  ...
  if (token.startsWith("额外:")) {
    ...
    if (payload === "开自动" || payload === "开启自动" || payload.toLowerCase() === "auto") {
