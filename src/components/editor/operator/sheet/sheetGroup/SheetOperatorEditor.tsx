  ...
  const otherGroups = useMemo(
    () =>
      existedGroups.filter(
        ({ name: existedName, opers }) => existedName !== name && !!opers?.length,
      ),
    [existedGroups, name],
  );
    ...
    Object.entries(needModifyGroups).forEach(([key, value]) => {
      if (key === "noneGrouped") {
        ...
        newOpers = newOpers?.concat(
          value.map(
            (name) => otherGroupOpers?.find(({ name: existedName }) => existedName === name)!,
          ),
        );
        if (key !== name)
          submitGroupInSheet({
      ...
      <form className="mt-3" onSubmit={onSubmit}>
        <div className="max-h-96 overflow-y-auto overflow-x-hidden">
          <OperatorSelectorSkeleton
            icon="person"
            title={
              t.components.editor.operator.sheet.sheetGroup.SheetOperatorEditor.selected_operators
            }
          ...
          <OperatorSelectorSkeleton
            icon="person"
            title={
              t.components.editor.operator.sheet.sheetGroup.SheetOperatorEditor.unselected_operators
            }
              ...
              <div key={otherGroupName}>
                <div className="flex flex-row-reverse items-center">
                  ...
                  <Button
                    ...
                    icon="arrow-top-left"
                    title={
                      t.components.editor.operator.sheet.sheetGroup.SheetOperatorEditor.select_all
                    }
                    onClick={() =>
                      opers?.forEach(({ name }) => {
