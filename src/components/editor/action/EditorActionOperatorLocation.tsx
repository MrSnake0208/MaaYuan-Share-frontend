...
interface EditorActionOperatorLocationProps extends EditorFieldProps<
  CopilotDocV1.Action,
  [number, number]
> {
...
export const EditorActionOperatorLocation = ({
  ...
  const {
    ...
    rules: {
      required:
        isRequired && t.components.editor.action.EditorActionOperatorLocation.location_required,
      validate: (v) => {
        // v being undefined is allowed because the `required` rule will handle it properly
        if (v) {
          if (
            !(Array.isArray(v) && v.length === 2 && v.every((i) => i >= 0 && Number.isFinite(i)))
          ) {
