...
export const handleDownloadJSON = (operationDoc: CopilotDocV1.Operation) => {
  ...
  const snakeCaseDoc = snakeCaseKeysUnicodeExceptSubtrees(operationDoc as any) as Record<
    string,
    unknown
  >;
...
const getSnakeCaseOperationDoc = async (
  ...
  try {
    ...
    return snakeCaseKeysUnicodeExceptSubtrees(rawDoc as any) as Record<string, unknown>;
  } catch (error) {
    console.error(error);
    AppToaster.show({
...
export const handleLazyDownloadJSON = async (id: number, title: string, tags?: string[]) => {
...
export const handleLazyDownloadSimingJSON = async (id: number, title: string) => {
  ...
  const sanitizedDoc = stripOperationExportFields(snakeCaseDoc);
  const simingActionsCandidate = sanitizedDoc["siming_actions"] ?? sanitizedDoc["actions"];
